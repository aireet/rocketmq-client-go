...
func NewNamesrv(resolver primitive.NsResolver, config *remote.RemotingClientConfig, group string) (*namesrvs, error) {
	...
	nameSrvClient := remote.NewRemotingClient(config, group)
	return &namesrvs{
		srvs:             addr,
		lock:             new(sync.Mutex),
